  ...
  const handleTransmit = async (e) => {
    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          access_key: "67087e9e-510d-4e85-a487-03b3f6393aab", // Replace with actual key
          name: formData.name,
          email: formData.email,
          company: formData.company,
          message: formData.payload,
          subject: `New Message from ${formData.name} via Juveon Labs`,
        }),
      });

      const result = await response.json();
      if (result.success) {
        setTransmitStatus('SUCCESS. LOG_SAVED.');
        ...
        setTransmitStatus('FAILED. ' + (result.message || 'INVALID_KEY'));
      }
    } catch (error) {
      console.error("Submission error:", error);
      setTransmitStatus('ERROR. UPLINK_LOST.');
    }
